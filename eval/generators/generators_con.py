...
class ImplicitGenerator3d(nn.Module):
    ...
    def forward(self, z, img_pitch, img_yaw, img_size, fov, ray_start, ray_end, num_steps, h_stddev, v_stddev, h_mean, v_mean, hierarchical_sample, sample_dist=None, lock_view_dependence=False, mode='', **kwargs):
        """
    ...
    def generate_avg_frequencies(self):
        ...
        # latent = torch.randn((10000, self.z_dim), device=self.siren.device)
        latent = torch.randn((10000, self.z_dim), device=self.siren.device)
        with torch.no_grad():
            frequencies, phase_shifts = self.siren.mapping_network.forward(latent)
    ...
    def staged_forward(self, z, img_pitch, img_yaw, img_size, fov, ray_start, ray_end, num_steps, h_stddev, v_stddev, h_mean, v_mean, psi=1, lock_view_dependence=False, max_batch_size=50000, depth_map=False, near_clip=0, far_clip=2, sample_dist=None, hierarchical_sample=False, mode='', **kwargs):
        """
        ...
        with torch.no_grad():
            ...
            if z.shape[1] < 4000:
                ...
                raw_frequencies = z[..., :z.shape[-1]//2]
                raw_phase_shifts = z[..., z.shape[-1]//2:]
        ...
        return pixels, depth_map, pitch, yaw
